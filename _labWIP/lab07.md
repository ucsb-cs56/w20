---
assigned: 2020-03-03 17:00
desc: Spring Boot Skills: Database and Users Table
...
org: ucsb-cs56-w20
ready: false
starter: https://github.com/ucsb-cs56-f19/STARTER_lab07
oauth_lab: lab06
---

<div style="display:none" >
Look here for formatted version: http://ucsb-cs56.github.io/w20/lab/lab07
Look here for formatted version: http://ucsb-cs56.github.io/w20/labWIP/lab07
</div>
...
In lab {{page.oauth_lab}}, there were two steps for:
* Setting up GitHub OAuth on localhost
* Setting up GitHub OAuth on Heroku

Please do those steps now for this application.  That includes setting up this
application on Heroku.

# Step 3:  Understanding SQL databases 
...
# Step 3: The various database packages

There are a lot of different technologies and packages that we'll be using here to
get access to databases, and it's easy to
get lost amid all the different names.

## H2 vs. Postgres

First, let's establish that we are using two different database technologies:

* H2, a temporary in-memory database when running on localhost
* Postgres, and long-term real database when running on Heroku

## More detail on H2 vs. Postgres
...
* If we exceeded the data storage capacity of the free tier, then we'd have to pay for it, but that won't happen
  in this lab assignment.   The free tier gives us up to 10,000 rows across all the tables in our database.
  

## The middleware layers

There are a lot of other technologies sitting between our Java code and the actual database.

* There is Spring Boot itself.
* There are also a variety of kinds of "middleware": Hibernate, JPA and JDBC are all layers in between the SQL database and our application code.
...
# Step 5: Adding additional `properties` files
...
# Step 6: Additional dependencies for working with database
...
# Step 7: Test that nothing is broken locally
...
# Step 8:  Adding a table for Users
...
## Step 8a:  Create a feature branch `xxAppUsersTable`
...
## Step 8c:  Creating a `@Repository` for users
...
## Step 8d:  Creating a `UsersController`
...
## Step 8e:  Creating a view for the Users
...
## Step 9: Add a navigation bar item for Users
...
### Step 9a: Provision an Heroku Postgres database addon
